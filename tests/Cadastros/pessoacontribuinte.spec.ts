...
test('Cadastro de Clientes Contribuintes', async ({ page }) => {
    // ⏳ Aumenta o timeout deste teste específico para 60 segundos (60000ms)
    ...
    // Define a promessa de captura da API imediatamente antes da ação de envio (clique)
    const salvarPessoaPromise = page.waitForResponse(
      (response) =>
        ...
        response.status() >= 200 &&
        response.status() < 300,
      { timeout: 15000 } // Espera no máximo 15s pela resposta da API após clicar
    );
    ...
    // Aguarda a resposta do salvar
    const salvarPessoaResponse = await salvarPessoaPromise;     
    const urlCompletaPost = salvarPessoaResponse.url();
