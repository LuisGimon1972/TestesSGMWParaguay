...
test('Exclusão de datos marcas', async ({ page }) => {
    ...
    if (trashIcons === 0) {
      ...
      console.log(`     ✅ Marca selecionada para exclusão: ${marca || 'Desconhecido'}`);              
      ...
      const codigoMarca = await primeiraLinha.nth(2).textContent(); // exemplo: coluna 1
      const codigoLimpo = codigoMarca?.trim();
      ...
      console.log('✅ Clicou em no diálogo de confirmação');

      const deleteResponse = await page.waitForResponse((response) =>
      response.url().includes(`/api/marca/${codigoLimpo}`) &&
      response.request().method() === 'DELETE');
      expect([200, 204]).toContain(deleteResponse.status());

      const getExcluidoResponse = await page.request.get(`/api/marca/${codigoLimpo}`);
