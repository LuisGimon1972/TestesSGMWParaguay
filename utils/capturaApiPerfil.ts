...
export async function capturarRequisicaoApiPerfil(page: Page): Promise<{ payload: any; resposta: any }> {
  return new Promise((resolve, reject) => {
    let payloadCapturado: any = null;
    let respostaCapturada: any = null;

    page.on('request', request => {
    if (['POST', 'PUT', 'PATCH'].includes(request.method()) && request.url().includes('/api/perfil')) {
    try {
    ...
    if (['POST', 'PUT', 'PATCH'].includes(response.request().method()) && response.url().includes('/api/perfil')) {
    try {
