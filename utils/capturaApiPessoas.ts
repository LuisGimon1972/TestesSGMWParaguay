...
export async function capturarRequisicaoApiCadastroPessoas(page: Page): Promise<{ payload: any; resposta: any }> {
  return new Promise((resolve, reject) => {
    let payloadCapturado: any = null;
    let respostaCapturada: any = null;

    page.on('request', request => {
    if (['POST'].includes(request.method()) && request.url().includes('/api/py/pessoa')) {
    try {
    ...
    if (['POST', 'PUT', 'PATCH'].includes(response.request().method()) && response.url().includes('/api/py/pessoa')) {
    try {
